# app/main.py - v1.0.1 with timezone button callback support
import logging
import os
...
def main():
    ...
    # --- Conversation: Consultation Booking (v1.0.1 with timezone buttons) ---
    consult_conv = ConversationHandler(
        ...
        states={
            ...
            consultation.SLOT_SELECT: [
                # v1.0.1: Timezone is now selected via inline buttons
                CallbackQueryHandler(consultation.timezone_selected_callback, pattern="^tz_")
            ],
            consultation.TIME: [
                MessageHandler(
                    tg_filters.TEXT & ~tg_filters.COMMAND & ~custom_filters.home_button & ~custom_filters.admin_button_hardcoded, 
            ...
            consultation.PROBLEM: [
                # Can receive text OR slot callback
                MessageHandler(
                    tg_filters.TEXT & ~tg_filters.COMMAND & ~custom_filters.home_button & ~custom_filters.admin_button_hardcoded, 
        ...
        fallbacks=[
            ...
            # Fallback callbacks for timezone and slot selection
            CallbackQueryHandler(consultation.timezone_selected_callback, pattern="^tz_"),
            CallbackQueryHandler(consultation.slot_selected_callback, pattern="^slot_")
        ],
        per_user=True,
    ...
    print("✅ All handlers registered (v1.0.1 with timezone buttons)")
    app.run_polling()
